...
/*   Updated: 2024/06/24 18:40:23 by lilin            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		bool to_append)
{
	...
	while (shell->envp[i])
	{
		...
		if (ft_strcmp(shell->env_key, key) == 0)
		{
			...
			if (to_append)
			{
				shell->env_value = ft_substr(shell, shell->envp[i],
						index_of_char(shell->envp[i], '=') + 1,
						ft_strlen(strchr(shell->envp[i], '=') - 1));
				shell->envp[i] = append(shell, shell->envp[i], shell->env_value,
						BOTH);
			}
...
bool	parse_check_export_args(char *arg, int *custom_errno, int *exit_status,
		int *to_append)
{
	if (arg[0] == '-')
	{
...
void	add_new_entry(t_minishell *shell, char *arg, int to_append)
{
	...
	if (to_append)
	{
		...
		shell->new_envp[old_size] = append(shell, shell->new_envp[old_size],
				shell->new_value, BASE);
	}
	else
...
int	ft_export(t_minishell *shell, char *arg, int *custom_errno)
{
	...
	if (!parse_check_export_args(arg, custom_errno, &exit_status, &to_append))
		return (exit_status);
	shell->new_key = ft_substr(shell, arg, 0, index_of_char(arg, '=')
			- to_append);
	shell->new_value = ft_substr(shell, arg, index_of_char(arg, '=') + 1,
			ft_strlen(strchr(arg, '=') - 1));
